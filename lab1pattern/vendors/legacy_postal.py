...
class OldPostalService:
    ...
    def compute_fare(self, weight_oz: float, zip_code: int) -> dict:
        ...
        if zip_code == 0 or zip_code > 99999:
            raise OldPostalSocketTimeout(
                f"Gateway timeout contacting ZIP routing mainframe for {zip_code}"
            )

        # Rate: $5.00 (500 cents) base + $0.25 (25 cents) per ounce
        fee_cents = int(500 + (weight_oz * 25))

        # Transit: 48 hours base + 12 hours for each 50 ounces
